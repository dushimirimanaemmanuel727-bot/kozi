...
export default async function WorkerProfilePage({ params }: PageProps) {
  ...
  const workerProfile = {
    ...workerProfileRow,
    photoUrl: workerProfileRow.photourl, // Transform database column name to frontend-friendly name
    user: {
  ...
  try {
    await query(
      'UPDATE "WorkerProfile" SET viewcount = viewcount + 1 WHERE id = $1',
      [id]
    );
  } catch (error) {
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-8 mb-8">
          <div className="flex flex-col lg:flex-row items-center lg:items-start space-y-6 lg:space-y-0 lg:space-x-8">
            ...
            <div className="flex-1 text-center lg:text-left">
              <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2">{workerProfile.user.name}</h1>
              <div className="flex flex-wrap justify-center lg:justify-start gap-3 lg:gap-4 text-sm text-gray-600 mb-4">
                ...
                <span className="flex items-center">
                  ...
                  {workerProfile.user.district || "Location not specified"}
                </span>
                <span className="flex items-center">
                  <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  {workerProfile.experienceYears} years experience
                </span>
              </div>
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-4 mb-6">
                <div className="flex items-center bg-yellow-100 px-3 py-2 rounded-lg">
                  ...
                  <span className="font-semibold text-yellow-800">{workerProfile.rating.toFixed(1)}</span>
                  <span className="text-yellow-700 ml-1">({workerProfile.reviewCount} reviews)</span>
                </div>
                ...
                <div className="flex items-center bg-blue-100 px-3 py-2 rounded-lg">
                  ...
                  <span className="font-semibold text-blue-800">{(workerProfile as any).viewCount || 0} views</span>
                </div>
              </div>
              ...
              <div className="flex gap-4 justify-center lg:justify-start">
                <a
                  href={`tel:${workerProfile.user.phone}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <p className="text-gray-700 leading-relaxed">
                {workerProfile.bio || "No bio provided yet."}
              </p>
            </div>
            ...
            {workerProfile.skills && (
              <div className="bg-white rounded-xl shadow-sm border p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {workerProfile.skills.split(',').map((skill: string, index: number) => (
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center text-gray-600">
                  ...
                  <span>{workerProfile.user.district || "Location not specified"}</span>
                </div>
                {workerProfile.user.languages && (
                  <div className="flex items-center text-gray-600">
                    <svg className="w-5 h-5 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" />
                    </svg>
                    <span>{workerProfile.user.languages}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900">{workerProfile.category}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Experience</span>
                  <span className="font-medium text-gray-900">{workerProfile.experienceYears} years</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Availability</span>
                  <span className="font-medium text-gray-900">{workerProfile.availability.replace('_', ' ')}</span>
                </div>
                {workerProfile.minMonthlyPay && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Min Monthly Pay</span>
                    <span className="font-medium text-gray-900">RWF {workerProfile.minMonthlyPay.toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-600">Live-in Available</span>
                  <span className="font-medium text-gray-900">{workerProfile.liveIn ? 'Yes' : 'No'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center">
                  <svg className={`w-5 h-5 mr-3 ${(workerProfile as any).nationalId ? 'text-green-500' : 'text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span className="text-gray-700">National ID {(workerProfile as any).nationalId ? 'Verified' : 'Not Provided'}</span>
                </div>
                <div className="flex items-center">
                  <svg className={`w-5 h-5 mr-3 ${(workerProfile as any).passportUrl ? 'text-green-500' : 'text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span className="text-gray-700">Passport {(workerProfile as any).passportUrl ? 'Verified' : 'Not Provided'}</span>
                </div>
              </div>
            </div>
          </div>
