...
export default function EmployerDashboard() {
  ...
  const [workersApiError, setWorkersApiError] = useState<boolean>(false);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // Fetch available workers - with error handling to prevent rate limiting
        try {
          ...
          if (workersResponse.ok) {
            ...
            setWorkersApiError(false);
          } else {
            console.error('Workers API failed:', workersResponse.status);
            setWorkersApiError(true);
            // Don't throw error for workers API failure to prevent breaking the whole dashboard
          }
        } catch (workersErr) {
          console.error('Workers API error:', workersErr);
          setWorkersApiError(true);
          // Don't throw error for workers API failure to prevent breaking the whole dashboard
        }
                  ...
                  fetch("/api/workers")
                    .then(response => {
                      if (response.ok) {
                        ...
                        throw new Error(`Workers API failed: ${response.status}`);
                      }
                    })
                    .then(data => {
