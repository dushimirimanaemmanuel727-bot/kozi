  ...
  try {
    ...
    const jobResult = await query(
      `SELECT 
        ...
        j.category,
        j.description,
        j.payamount as budget,
        j.location as district,
        j.status,
...
export async function PUT(req: NextRequest, { params }: Params) {
  try {
    ...
    const existingJobResult = await query(
      'SELECT id, employerid as "employerId", status FROM "Job" WHERE id = $1',
      [id]
    );
    ...
    if (budget !== undefined) { fields.push(`payamount = $${paramIndex++}`); values.push(Number(budget)); }
    if (district !== undefined) { fields.push(`location = $${paramIndex++}`); values.push(district); }
...
export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    ...
    // Check if job exists and user owns it
    const jobResult = await query('SELECT id, employerid as "employerId" FROM "Job" WHERE id = $1', [id]);
    const job = jobResult.rows[0];
