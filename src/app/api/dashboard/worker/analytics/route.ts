  ...
  try {
    ...
    const trendResult = await query(
      `SELECT DATE_TRUNC('month', appliedat) as month, COUNT(*) as count 
       FROM "Application" 
       WHERE workerid = $1 AND appliedat >= $2
    ...
    // Get worker's location from profile
    const workerLocation = workerProfile?.location;
    ...
    if (workerProfile) {
      const recommendedResult = await query(
        `SELECT j.*, u.name as employer_name, 
         ...
         WHERE j.status = 'active' AND (j.category = $1 OR j.location = $2)
         ORDER BY j.createdat DESC LIMIT 6`,
        [workerProfile.category, workerLocation]
      );
