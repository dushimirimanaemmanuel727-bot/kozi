  ...
  const [
    ...
    query`
      SELECT 
        status,
        COUNT(*) as _count,
        COALESCE(AVG(payamount), 0) as _avg_budget
      FROM "Job"
      GROUP BY status
    `.then(result => result.rows),
    ...
    query`
      SELECT 
        ...
        (SELECT COUNT(*) FROM "Application" WHERE appliedat >= NOW() - INTERVAL '24 hours') as new_applications_today,
        ...
        (SELECT COUNT(*) FROM "Application" WHERE appliedat >= NOW() - INTERVAL '7 days') as new_applications_week,
        ...
        (SELECT COUNT(*) FROM "Application" WHERE appliedat >= NOW() - INTERVAL '30 days') as new_applications_month
    `.then(result => result.rows),
    ...
    query`
      ...
      WHERE a.appliedat >= NOW() - INTERVAL '24 hours'
  ...
  return (
    <SystemMonitoring 
      ...
      data={{
        ...
        jobStats,
        applicationStats,
        verificationStats: [], // No verification table exists
        reviewStats,
        notificationStats,
        systemHealth: health,
        recentActivity,
