...
export interface User {
  ...
  district?: string;
  languages?: string;
  // Database uses lowercase column names
  passwordhash?: string;
  suspended: boolean;
...
export async function createUser(userData: CreateUserData): Promise<User> {
  ...
  const result = await query(
    `INSERT INTO "User" (
      name, phone, email, role, passwordhash, suspended, createdat, updatedat
    ) VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
    RETURNING *`,
    [name, phone, email, role, passwordhash, false]
  );
...
export async function verifyPassword(phone: string, password: string): Promise<User | null> {
  ...
  // Database uses lowercase passwordhash
