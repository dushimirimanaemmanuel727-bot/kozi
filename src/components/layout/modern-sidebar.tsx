...
export default function ModernSidebar({ onClose }: ModernSidebarProps) {
  ...
  const getColorClasses = (color: string, isActive: boolean) => {
    const colors: { [key: string]: { active: string; inactive: string } } = {
      ...
      indigo: {
        active: "bg-indigo-600 text-white shadow-indigo-600/50",
          ...
          <div className="grid grid-cols-1 gap-3">
            {quickActions.map((action) => (
              <Link
                ...
                className={`group flex items-center px-4 py-3 ${action.bgColor} ${action.textColor} rounded-2xl ${action.hoverColor} transition-all duration-300 ease-out hover:scale-[1.02] hover:shadow-md`}
              >
