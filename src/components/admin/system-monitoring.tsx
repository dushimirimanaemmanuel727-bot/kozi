...
export function SystemMonitoring({ session, data }: SystemMonitoringProps) {
  ...
  const totalUsers = data.userStats.reduce((sum, stat) => sum + stat._count.id, 0);
  const totalJobs = data.jobStats.reduce((sum, stat) => sum + stat._count.id, 0);
  const totalApplications = data.applicationStats.reduce((sum, stat) => sum + stat._count.id, 0);
  const pendingVerifications = data.verificationStats.find(s => s.status === 'PENDING')?._count.id || 0;
